...
- 본 프로젝트는 시계열 데이터인 비트코인 데이터를 활용하여, 가격을 예측하고 매수 타이밍을 고려하는 딥러닝 모델을 설계하고자 한다.
- 선택한 전략: GRU 기반 모델을 채택. thershold는 0.6 이상일 때 투자를 하도록 하였다. 
- 전략의 장단점: 장점으로는 상대적으로 높은 thershold를 설정해 확실한 경우에만 투자를 할 수 있도록 한 것, 단점으로는 지속적인 가격 상승임에도 불구하고, 보수적인 접근만을 허용해 기회 손실이 많다. 
- 수수료 영향: 0.002로 설정하여, 기존보다는 조금 높은 수수료이다. 수수료 부담은 2회에 걸친 약 $25로 큰 양은 아닌 것으로 사료된다. 보수적인 접근에 따른 적은 시장 진입을 통해 큰 수입은 아니니만 안정적인 수익률을 유지했을 것이다. 
...
- 아키텍처 선택 이유: 시계열 데이터를 사용할 수 있으며, 기존 LSTM보다 시간이 적게 소요되기 때문이다. 
- 하이퍼파라미터 튜닝: hidden_size는 64로 설정하여 과적합을 방지하도록 하였다.learning_rate는 0.002로 설정하여 학습 초반 수렴 속도를 높이고, 조기종료를 통해 과적합을 방지할 수 있게 하였다. 또한 threshold를 0.6으로 상향 조정하여 신뢰도가 높은 예측에 대해서만 트레이딩이 이루어지도록 설계하였다. 즉, 보수적인 트레이딩 전략을 구성하였다. 
- 예제 모델과의 차이점: 우선 채택한 아키텍처가 다르다. 이 모델은 예제 모델보다 높은 수익률을 냈음에도 불구하고, 조금 더 보수적인 성향을 지닌다. 따라서, 예제 모델에 비해 비교적 정확도가 높은 시기에 매매를 시도하는 확률 기반 모델이라 할 수 있다.


## 4. 개선 방향**

- 모델의 한계점: 단순히 하락과 상승만 예상할 수 있으며, 원인 파악을 하기 어렵다. 또한 시장 외부적 상황(정치 등)을 고려하기 힘들기에 추가적인 학습이 필요하다.
- 추가 실험 아이디어: 앙상블 전략을 통해 신뢰도를 향상시킬 수 있을 것 같다. 즉, 제공된 예제모델과 본 모델을 합하여 보수적이면서도 꾸준히 투자를 할 때 좋은 모델이 될 것 같다. 
- 실전 적용 시 고려사항: 실제 사회 상황을 고려해야한다. 호재와 악재 등을 생각해야하기 때문이다. 또한, 지속적인 테스트를 통해 기능의 성능을 검증해야한다.
